...
public class Uml2autojava extends AbstractAcceleoGenerator {
	...
	private String decorationModels = null;
	...
	public Uml2autojava(URI modelURI, File targetFolder,
			List<? extends Object> arguments, String sDecorators)
			throws IOException {
	 ...
	 * @generated
	 */
	public Uml2autojava(EObject model, File targetFolder,
			List<? extends Object> arguments) throws IOException {
	...
	public static void main(String[] args) {
		try {
			if (args.length < 2) {
				...
				Uml2autojava generator = new Uml2autojava(modelURI, folder,
						arguments);
	...
	protected void postInitialize() {
		...
		if (!devMode) {
			if (decorationModels != null) {
				...
				try {
					...
					for (int i = 0; i < decorators.length; i++) {
						if (decorators[i] != null && !decorators[i].trim().equals("")) {
			...
			final List<URI> parameters = new ArrayList<URI>();
			if (DEV_MODE_DECORATOR_PATH != null && !DEV_MODE_DECORATOR_PATH.trim().equals("")) {
	...
	public List<String> getProperties() {
		...
		propertiesFiles
				.add("org/eclipse/umlgen/gen/autojava/properties/default.properties");
		propertiesFiles
				.add("org/eclipse/umlgen/gen/autojava/properties/types.properties");
		propertiesFiles
				.add("org/eclipse/umlgen/gen/autojava/properties/primitiveTypes.properties");
	...
	public void registerPackages(ResourceSet resourceSet) {
		...
		if (!isInWorkspace(org.eclipse.umlgen.dsl.asl.AslPackage.class)) {
			resourceSet
					.getPackageRegistry()
					.put(org.eclipse.umlgen.dsl.asl.AslPackage.eINSTANCE
							.getNsURI(),
		...
		if (!isInWorkspace(org.eclipse.umlgen.dsl.asl.configurations.ConfigurationsPackage.class)) {
			resourceSet
					.getPackageRegistry()
					.put(org.eclipse.umlgen.dsl.asl.configurations.ConfigurationsPackage.eINSTANCE
							.getNsURI(),
		...
		if (!isInWorkspace(org.eclipse.umlgen.dsl.eth.EthPackage.class)) {
			resourceSet
					.getPackageRegistry()
					.put(org.eclipse.umlgen.dsl.eth.EthPackage.eINSTANCE
							.getNsURI(),
	...
	public AbstractAcceleoPropertiesLoaderService getPropertiesLoaderService(
		...
		if (EMFPlugin.IS_ECLIPSE_RUNNING) {
			...
			return new ObeoNetworkBundlePropertiesLoaderService(acceleoService,
					bundle);
		}
